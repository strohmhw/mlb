...
daily_model = daily_model.merge(daily_team_batters_vsAll_advanced_df[['Tm', 'vs_L_Adj', 'vs_R_Adj']], how='left', left_on='Home Symbol', right_on='Tm')
daily_model = daily_model.merge(daily_team_batters_vsAll_advanced_df[['Tm', 'vs_L_Adj', 'vs_R_Adj']], how='left', left_on='Away Symbol', right_on='Tm')
daily_model = daily_model.drop(['Tm_x', 'Tm_y'], axis=1)
daily_model['BsR_Sc_Season_Home_Split'] = (daily_model['BsR_Sc_Season_x'] * daily_model['vs_L_Adj_x']).where(daily_model['Home pHand'] == 'L', (daily_model['BsR_Sc_Season_x'] * daily_model['vs_R_Adj_x']))
daily_model['BsR_Sc_Season_Away_Split'] = (daily_model['BsR_Sc_Season_y'] * daily_model['vs_L_Adj_y']).where(daily_model['Away pHand'] == 'L', (daily_model['BsR_Sc_Season_y'] * daily_model['vs_R_Adj_y']))
...
daily_model['BsR_Al_Season_Home'] = daily_model['BsR_Al_Season_x'] * daily_model['xFIP_adj_Home']
daily_model['BsR_Al_Season_Away'] = daily_model['BsR_Al_Season_y'] * daily_model['xFIP_adj_Away']

# Daily Win% With xFIP Adjustments & L/R Split Adjustments
daily_model['Exp_Win_%_Home_Final'] = np.power(daily_model['BsR_Sc_Season_Home_Split'],1.83) / (np.power(daily_model['BsR_Sc_Season_Home_Split'],1.83) + np.power(daily_model['BsR_Al_Season_Home'],1.83))
daily_model['Exp_Win_%_Away_Final'] = np.power(daily_model['BsR_Sc_Season_Away_Split'],1.83) / (np.power(daily_model['BsR_Sc_Season_Away_Split'],1.83) + np.power(daily_model['BsR_Al_Season_Away'],1.83))

# Game Specific Win% w/ Home Field Advantage
daily_model['Exp_Win_%_Home_Driver'] = (daily_model['Exp_Win_%_Home_Final'] / (daily_model['Exp_Win_%_Home_Final'] + daily_model['Exp_Win_%_Away_Final'])) * 1.08
daily_model['Exp_Win_%_Away_Driver'] = (daily_model['Exp_Win_%_Away_Final'] / (daily_model['Exp_Win_%_Away_Final'] + daily_model['Exp_Win_%_Home_Final'])) * .92
...
daily_model.to_csv('daily_model_' + today.strftime("%Y-%m-%d") + '.csv')
>>>>>>> 7bb60491998a7e2fa017ac9af974c3893ca1a570:daily-script-final-work.py
